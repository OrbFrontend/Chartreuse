...
1. The dataset so far is mostly gathered from _RP_ logs where slop is more prominent, 
so zero-shot short story writing will likely slip past the classifier. Also because short stories take the
shape of human writing (from /r/WritingPrompts and the like), and I tip-toe around human writing.

2. Only works on Nvidia GPUs.
